...
def pose_text(frame):
    ...
    if frame != []:
       ...
       capture_pose_x, capture_pose_y, frame = cutpig.cut_coordinate(frame)
       if capture_pose_x is not None and capture_pose_y is not None:
           ...
           if rec_res != []:
               ...
               rec_res = re.sub(pattern,'',rec_res)
               #print(rec_res)
               rec_res = character_match(rec_res)
           else:
               print("没有正确字符！！！")
             ...
             print("data is an empty !!!")
    return capture_pose_x, capture_pose_y, rec_res
